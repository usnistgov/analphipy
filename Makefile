...
version-import: ## check version from python import
	-uv run --frozen python -c 'import analphipy; print(analphipy.__version__)'
...
install-kernel:  ## install kernel
	...
	--name analphipy-dev \
    --display-name "Python [venv: analphipy-dev]"
...
tuna-import	: ## Analyze load time for module
	uv run --frozen python -X importtime -c 'import analphipy' 2> tuna-loadtime.log
